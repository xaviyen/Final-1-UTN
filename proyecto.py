# ###########################################################################################################
# Proyecto UTN: ISP V1.0
# Integrantes:
# - Tomás Inti Malafiej
# - Celso Javier Caniu
# - Nicolás Ariel Ugarte
# - María Emilia Cao González
# - Sebastián Minotti
# ###########################################################################################################
...
def abrir_archivo():
    ...
    ruta = os.path.dirname(os.path.abspath(__file__))
    nom_archivo_a = fd.askopenfilename(
        title="Abrir archivo", initialdir=ruta, filetypes=tipo_archivo
    )

    arch_xlsx = os.path.join(ruta, nom_archivo_a)
    nombrehoja = "Datoscliente"
    indice = 0
    print(arch_xlsx)
    df = pd.read_excel(io=arch_xlsx, sheet_name=nombrehoja, header=indice)
    print(df.head(n=df["N° de cliente"].count()))
...
# Función para cargar los datos modificados en el treeview y en la DB
def f_modificar():
...
# Función que levanta los datos de la DB y los carga en el treeview
def f_consulta():
    global tree
    try:
        conexion()
...
# Función para exportar los datos de la DB a un excel
def f_exportar():
    ruta = os.path.dirname(os.path.abspath(__file__))
